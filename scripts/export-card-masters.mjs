...
const atlasMeta=await sharp('assets/card-art-atlas.webp').metadata();
...
for(const c of cards.slice(0,limit)){
  ...
  const art=await sharp('assets/card-art-atlas.webp').extract({left:c.art.col*tileW,top:c.art.row*tileH,width:tileW,height:tileH}).resize(1320,900,{fit:'cover',kernel:'lanczos3'}).sharpen(1.2).png().toBuffer();
